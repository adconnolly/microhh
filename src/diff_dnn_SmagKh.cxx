...
template<typename TF>
Diff_dnn_SmagKh<TF>::Diff_dnn_SmagKh(Master& masterin, Grid<TF>& gridin, Fields<TF>& fieldsin, Boundary<TF>& boundaryin, Input& inputin) :
...
/*    if (grid.get_spatial_order() != Grid_order::Second)
        throw std::runtime_error("Diff_dnn_SmagKh only runs with second order grids");*/
}

template<typename TF>
Diff_dnn_SmagKh<TF>::~Diff_dnn_SmagKh()
{
}

template<typename TF>
void Diff_dnn_SmagKh<TF>::init()
{
    boundary_cyclic.init();
}

template<typename TF>
Diffusion_type Diff_dnn_SmagKh<TF>::get_switch() const
{
    return swdiff;
}
...
#ifndef USECUDA
template<typename TF>
unsigned long Diff_dnn_SmagKh<TF>::get_time_limit(const unsigned long idt, const double dt)
{
    auto& gd = grid.get_grid_data();
...
#ifndef USECUDA
template<typename TF>
double Diff_dnn_SmagKh<TF>::get_dn(const double dt)
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Diff_dnn_SmagKh<TF>::create(Stats<TF>& stats)
{
    auto& gd = grid.get_grid_data();
...
#ifndef USECUDA
template<typename TF>
void Diff_dnn_SmagKh<TF>::exec(Stats<TF>& stats)
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Diff_dnn_SmagKh<TF>::exec_viscosity(Thermo<TF>& thermo)
{
    auto& gd = grid.get_grid_data();
...
#ifndef USECUDA
template<typename TF>
void Diff_dnn_SmagKh<TF>::create_stats(Stats<TF>& stats)
{
    const std::string group_name = "default";
...
template<typename TF>
void Diff_dnn_SmagKh<TF>::exec_stats(Stats<TF>& stats)
{
...
template<typename TF>
void Diff_dnn_SmagKh<TF>::diff_flux(Field3d<TF>& restrict out, const Field3d<TF>& restrict fld_in)
{
    auto& gd = grid.get_grid_data();
...
template class Diff_dnn_SmagKh<double>;
template class Diff_dnn_SmagKh<float>;
