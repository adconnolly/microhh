                ...
                for (int i=istart; i<iend; ++i)
                {
                    ...
                    for (int iz=-iv; iz<=iv; iz++)
                        for (int ix=-ih; ix<=ih; ++ix)
                                for (int iy=-ih; iy<=ih; ++iy)
                                {                                    
                                    x.index_put_({ijkbatch, 2*(iz+iv),ih+ix,ih+iy}, (uc[ijk+ix*ii+iy*jj+iz*kk]-ubar)*ivel_scale);
                                    x.index_put_({ijkbatch, 2*(iz+iv)+1,ih+ix,ih+iy},(vc[ijk+ix*ii+iy*jj+iz*kk]-vbar)*ivel_scale);
                                    x.index_put_({ijkbatch, 2*nv+(iz+iv),ih+ix,ih+iy},(wc[ijk+ix*ii+iy*jj+iz*kk]-wbar)*ivel_scale);
                                    x.index_put_({ijkbatch, 3*nv+(iz+iv),ih+ix,ih+iy},(b[ijk+ix*ii+iy*jj+iz*kk]-bbar)*ib_scale);
                                }
...
template<typename TF>
Diff_dnn_global<TF>::Diff_dnn_global(Master& masterin, Grid<TF>& gridin, Fields<TF>& fieldsin, Boundary<TF>& boundaryin, Input& inputin) :
    ...
    field3d_operators(master, grid, fields)
{
    ...
    Re    = inputin.get_item<TF>("diff", "Re"  , "", 20000.0);
    Ug    = inputin.get_item<TF>("diff", "Ug"  , "", 0.05);
    b0 = inputin.get_item<TF>("diff", "b0"  , "", -0.0044);
    ivel_scale =  std::sqrt(Re)/Ug;
    stress_scale = Ug*Ug/Re;
    ib_scale    = 1.0/(0-b0);
    dnnpath = inputin.get_item<std::string>("diff", "dnnpath"  , "", "C4_midReGridExtrap_global_4x1026Re1800_4x2052Re1800_0.pt");
...
template<typename TF>
Diff_dnn_global<TF>::~Diff_dnn_global()
{
}

template<typename TF>
void Diff_dnn_global<TF>::init()
{
    boundary_cyclic.init();
}

template<typename TF>
Diffusion_type Diff_dnn_global<TF>::get_switch() const
{
    return swdiff;
}
...
#ifndef USECUDA
template<typename TF>
unsigned long Diff_dnn_global<TF>::get_time_limit(const unsigned long idt, const double dt)
{
    auto& gd = grid.get_grid_data();
...
#ifndef USECUDA
template<typename TF>
double Diff_dnn_global<TF>::get_dn(const double dt)
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Diff_dnn_global<TF>::create(Stats<TF>& stats)
{
    auto& gd = grid.get_grid_data();
...
#ifndef USECUDA
template<typename TF>
void Diff_dnn_global<TF>::exec(Stats<TF>& stats)
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Diff_dnn_global<TF>::exec_viscosity(Thermo<TF>& thermo)
{
    auto& gd = grid.get_grid_data();
...
#ifndef USECUDA
template<typename TF>
void Diff_dnn_global<TF>::create_stats(Stats<TF>& stats)
{
    const std::string group_name = "default";
...
template<typename TF>
void Diff_dnn_global<TF>::exec_stats(Stats<TF>& stats)
{
...
template<typename TF>
void Diff_dnn_global<TF>::diff_flux(Field3d<TF>& restrict out, const Field3d<TF>& restrict fld_in)
{
    auto& gd = grid.get_grid_data();
...
template class Diff_dnn_global<double>;
template class Diff_dnn_global<float>;
